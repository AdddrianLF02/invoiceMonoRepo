...
export class Invoice {
  ...
  public removeItem(itemId: string): Invoice {
    if (!this.status.isDraft()) throw new Error('Cannot replace items unless invoice is in draft status');
    return this.copyWith({ items: [...this.items.filter(item => item.getId() !== itemId)] })
  }
