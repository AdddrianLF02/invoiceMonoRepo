...
describe('InvoiceItem', () => {
    it('should create an invoice item with valid data', () => {
        ...
        const quantity = 2;
        const unitPrice = Money_1.Money.fromFloat(100);
        const taxRate = 0.21;
        const subtotal = Money_1.Money.fromFloat(200); // 2 * 100
        const taxAmount = Money_1.Money.fromFloat(42); // 200 * 0.21
        const total = Money_1.Money.fromFloat(242); // 200 + 42
        // Act
        const invoiceItem = InvoiceItem_1.InvoiceItem.create(description, quantity, unitPrice, taxRate, subtotal, taxAmount, total);
        // Assert
    ...
    it('should throw an error when total does not match subtotal + taxAmount', () => {
        ...
        const unitPrice = Money_1.Money.fromFloat(100);
        const taxRate = 0.21;
        const subtotal = Money_1.Money.fromFloat(200);
        const taxAmount = Money_1.Money.fromFloat(42);
        const incorrectTotal = Money_1.Money.fromFloat(250); // Incorrect total
        // Act & Assert
        expect(() => {
            InvoiceItem_1.InvoiceItem.create(description, quantity, unitPrice, taxRate, subtotal, taxAmount, incorrectTotal);
        }).toThrow('Calculated total does not match subtotal + tax amount');
    });
});
//# sourceMappingURL=InvoiceItem.test.js.map